...
function generate_arg-path_arg-g2s-exec_arg-gir-pre_arg-gir-path {
    ...
    bash -c "${G2S_EXEC} ${GIR_PATH}/${GIR_NAME}.gir ${GIR_PRE_ARGS} | sed -f ${GIR_NAME}.sed | awk -f ${GIR_NAME}.awk > Sources/${NAME}/${GIR_NAME}.swift"
    bash -c "${G2S_EXEC} -o Sources/${NAME} -m ${GIR_NAME}.module ${GIR_PATH}/${GIR_NAME}.gir ${GIR_PRE_ARGS}"
