...
/// The `GLTextureClass` type acts as an owner of an underlying `GdkGLTextureClass` instance.
/// It provides the methods that can operate on this data type through `GLTextureClassProtocol` conformance.
/// Use `GLTextureClass` as a strong reference or owner of a `GdkGLTextureClass` instance.
///

open class GLTextureClass: GLTextureClassProtocol {
        /// Untyped pointer to the underlying `GdkGLTextureClass` instance.
    /// For type-safe access, use the generated, typed pointer `_ptr` property instead.
    public let ptr: UnsafeMutableRawPointer!
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// This creates an instance without performing an unbalanced retain
    /// i.e., ownership is transferred to the `GLTextureClass` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafeMutablePointer<GdkGLTextureClass>) {
        ptr = UnsafeMutableRawPointer(op)
    }
    ...
    /// i.e., ownership is transferred to the `GLTextureClass` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<GdkGLTextureClass>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: op))
    }
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// `GdkGLTextureClass` does not allow reference counting, so despite the name no actual retaining will occur.
    /// i.e., ownership is transferred to the `GLTextureClass` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<GdkGLTextureClass>) {
        ptr = UnsafeMutableRawPointer(op)
        // no reference counting for GdkGLTextureClass, cannot ref(_ptr)
    }

    /// Reference intialiser for a related type that implements `GLTextureClassProtocol`
    /// `GdkGLTextureClass` does not allow reference counting.
    /// - Parameter other: an instance of a related type that implements `GLTextureClassProtocol`
    @inlinable public init<T: GLTextureClassProtocol>(_ other: T) {
        ptr = other.ptr
        // no reference counting for GdkGLTextureClass, cannot ref(_ptr)
    }

    /// Do-nothing destructor for `GdkGLTextureClass`.
    deinit {
        // no reference counting for GdkGLTextureClass, cannot unref(_ptr)
    }
...
// MARK: no GLTextureClass properties

// MARK: no GLTextureClass signals
