...
/// The `ToplevelInterface` type acts as an owner of an underlying `GdkToplevelInterface` instance.
/// It provides the methods that can operate on this data type through `ToplevelInterfaceProtocol` conformance.
/// Use `ToplevelInterface` as a strong reference or owner of a `GdkToplevelInterface` instance.
///

open class ToplevelInterface: ToplevelInterfaceProtocol {
        /// Untyped pointer to the underlying `GdkToplevelInterface` instance.
    /// For type-safe access, use the generated, typed pointer `_ptr` property instead.
    public let ptr: UnsafeMutableRawPointer!
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// This creates an instance without performing an unbalanced retain
    /// i.e., ownership is transferred to the `ToplevelInterface` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafeMutablePointer<GdkToplevelInterface>) {
        ptr = UnsafeMutableRawPointer(op)
    }
    ...
    /// i.e., ownership is transferred to the `ToplevelInterface` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<GdkToplevelInterface>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: op))
    }
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// `GdkToplevelInterface` does not allow reference counting, so despite the name no actual retaining will occur.
    /// i.e., ownership is transferred to the `ToplevelInterface` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<GdkToplevelInterface>) {
        ptr = UnsafeMutableRawPointer(op)
        // no reference counting for GdkToplevelInterface, cannot ref(_ptr)
    }

    /// Reference intialiser for a related type that implements `ToplevelInterfaceProtocol`
    /// `GdkToplevelInterface` does not allow reference counting.
    /// - Parameter other: an instance of a related type that implements `ToplevelInterfaceProtocol`
    @inlinable public init<T: ToplevelInterfaceProtocol>(_ other: T) {
        ptr = other.ptr
        // no reference counting for GdkToplevelInterface, cannot ref(_ptr)
    }

    /// Do-nothing destructor for `GdkToplevelInterface`.
    deinit {
        // no reference counting for GdkToplevelInterface, cannot unref(_ptr)
    }
...
// MARK: no ToplevelInterface properties

// MARK: no ToplevelInterface signals
...
public extension ToplevelLayoutProtocol {
    ...
    @inlinable func getMaximized(maximize: UnsafeMutablePointer<gboolean>!) -> Bool {
        let rv = ((gdk_toplevel_layout_get_maximized(toplevel_layout_ptr, maximize)) != 0)
        return rv
    }
